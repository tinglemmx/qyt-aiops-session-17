...
def get_last_timestamp(session: Session):
    last_timestamp_row = session.query(CpuMemData.timestamp).order_by(CpuMemData.timestamp.desc()).first()
    return last_timestamp_row[0]

def get_hosts_list(session: Session):
    devices_list = [row[0] for row in session.query(CpuMemData.device_ip).distinct().all()]
    return devices_list
...
def get_mem_and_cpu_usage_list(session: Session, device_ip: str,last_timestamp):
    ...
    cpu_usage_list = []
    one_hour_ago = last_timestamp - timedelta(hours=1) - timedelta(minutes=1) - timedelta(seconds=10)
    ...
    cpu_usage_list = calculate_cpu_usage(cpu_total_list,cpu_idle_w_iowait_list)
...
def generating_line_chart_per_device(session: Session,device_ip,last_timestamp):
    timestamp_list,mem_usage_list,cpu_usage_list = get_mem_and_cpu_usage_list(session,device_ip,last_timestamp)
...
def generating_line_chart(session: Session):
    ...
    for device_ip in devices_list:
        cpu_usage_info_list,mem_usage_info_list = generating_line_chart_per_device(session,device_ip,last_timestamp)
        mhost_cpu_usage_info_list.append (cpu_usage_info_list)
        mhost_mem_usage_info_list.append (mem_usage_info_list)
    draw_line_chart(mhost_cpu_usage_info_list,'记录时间','百分比',"CPU利用率","cpu_usage.png")
    draw_line_chart(mhost_mem_usage_info_list,'记录时间','百分比',"MEM利用率","mem_usage.png")
